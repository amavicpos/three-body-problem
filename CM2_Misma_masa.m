%% Extra figure: Three bodies with the same mass and Center of Mass' analysis
...
%% DEFINE BODIES
m=[m1 m2 m3]; % masses
r0=[[-1.7276e+10 -1.1896e+11 0]' [3.5333e+10 -2.5641e+10 0]' [-1.8056e+10 1.4460e+11 0]']; % initial r % !!! Different r0, v0
v0=[[-3.9863e+03 1.8228e+04 0]' [-1.9443e+03 -2.5307e+04 0]' [5.9306e+03 7.0784e+03 0]']; % initial v
...
x2(:,1)=r00(1,1:2:3)' ; y2(:,1)=r00(2,1:2:3)' ; z2(:,1)=r00(3,1:2:3)' ;
vx2(:,1)=v00(1,1:2:3)' ; vy2(:,1)=v00(2,1:2:3)' ; vz2(:,1)=v00(3,1:2:3)' ;
K(:,1)=K00' ; U(:,1)=U00' ;
...
for i=1:NS-1
   ...
   vx2(:,i+1)=vx2(:,i)+(fx(1:2:3)./(m(1:2:3)'))*dt ; %Two stars' vectors
   vy2(:,i+1)=vy2(:,i)+(fy(1:2:3)./(m(1:2:3)'))*dt ;
   vz2(:,i+1)=vz2(:,i)+(fz(1:2:3)./(m(1:2:3)'))*dt ;
   x2(:,i+1)=x2(:,i)+vx2(:,i+1)*dt ;
   y2(:,i+1)=y2(:,i)+vy2(:,i+1)*dt ;
   z2(:,i+1)=z2(:,i)+vz2(:,i+1)*dt ;
   rCM2(:,i+1)=sum(m(1:2:3).*[x2(:,i+1)'; y2(:,i+1)'; z2(:,i+1)'],2)/sum(m(1:2:3)) ;
   vCM2(:,i+1)=sum(m(1:2:3).*[vx2(:,i+1)'; vy2(:,i+1)'; vz2(:,i+1)'],2)/sum(m(1:2:3)) ;
   ...
   if rem(i,5000)==0 %Animated plot
       ...
       title('Evolución del sistema'); legend('Estrellas', 'Centro de masas total', 'CM estrellas exteriores');
...
legend('Estrella Kepler-16A','Estrella Kepler-16A','Estrella Kepler-16A','Centro de masas total','CM estrellas exteriores');
